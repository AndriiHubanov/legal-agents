...
12. [Поточні обмеження та ідеї для покращення](#12-поточні-обмеження-та-ідеї-для-покращення)
...
1. **Інтерактивний режим для Agent 1** — замість AI-симуляції запитань, реально питати у користувача через чат-інтерфейс (вже реалізовано через Web UI).
...
8. **Async пайплайн** — паралельний запуск Agent 1 і Agent 2 (незалежні).
9. **Розширення типів документів** — зустрічний позов, заява про зміну предмету, мирова угода.
10. **Інтеграція з реєстром** — автоматична перевірка статусу справи після подачі.
11. **Сесії з відновленням** — зберігати сесії Web UI між перезапусками сервера (зараз в пам'яті).
...
# або вказати інший порт
python main.py server --port 8080
```

Браузер відкривається автоматично на `http://127.0.0.1:8000`.
...
| Файл | Призначення |
|------|------------|
| `server.py` | FastAPI-сервер: роути, сесійне сховище, захоплення логів |
| `process.html` | Веб-інтерфейс (один HTML-файл, vanilla JS, без збірки) |
| `shared/file_processor.py` | Витяг тексту з PDF / DOCX / TXT |
...
| `GET` | `/api/download/{id}` | Завантажити готовий .docx |

---

### Потік даних із файловим вводом

```
Користувач завантажує файли у process.html
         │
         ▼
POST /api/run  (multipart/form-data)
         │
         ▼
server.py: await file.read() → extract_text_from_bytes(filename, bytes)
  ├── .pdf  → pypdf.PdfReader → сторінки в текст
  ├── .docx → python-docx Document → параграфи в текст
  └── .txt  → decode UTF-8 / CP1251
  │
  Обрізка до 8 000 символів/файл (~2 000 токенів)
  Формат: "[назва_файлу]\n<витягнутий текст>"
         │
         ▼
...
pipeline_v2.py → agent1.process(
    raw_situation=...,
    supporting_docs=state.supporting_docs   ← тільки на ітерації 0
)
         │
         ▼
Agent 1: _build_user_message() додає секцію
  ## ДОДАТКОВІ ДОКУМЕНТИ (завантажені користувачем)
  ### Документ 1
  [contract.pdf]
  Договір оренди від 01.01.2023...
         │
         ▼
Agent 1 формує збагачений CaseDescription
  (з кращими ключовими словами, правовими підставами)
         │
         ▼
Agent 2 (RAG): ChromaDB-пошук по збагаченому CaseDescription
  → точніші результати судової практики
```

---

### Захоплення логів у браузері

Пайплайн (`pipeline_v2.py`, агенти) логує через `shared/logger.py` → `RichHandler`.
Сервер додає `_QueueLogHandler` до кореневого Python-логера перед запуском пайплайну:

```python
class _QueueLogHandler(logging.Handler):
    def emit(self, record):
        self.q.put(self.format(record))   # → SSE-черга

# Після завершення:
root_logger.removeHandler(handler)
session.log_queue.put(None)   # sentinel → закриває SSE-з'єднання
```

Браузер читає логи через `EventSource('/api/logs/{id}')`.
Термінал отримує паралельно Rich-форматований вивід (він не відключається).

---

### Сесійне сховище

Зберігається в пам'яті (`dict[str, SessionInfo]`) — тільки для локального використання.
При перезапуску сервера всі сесії скидаються. Завантажити файл, що вже існує на диску, можна через CLI.
...
class SessionInfo:
    ...
    log_queue: queue.Queue    # SSE-черга
...
| Файл | Зміна |
|------|-------|
| `shared/models.py` | `PipelineState` + поле `supporting_docs: list[str]` |
| `orchestrator/state.py` | `create_state()` + параметр `supporting_docs` |
| `agent1_intake/intake_agent.py` | `process()` + параметр `supporting_docs`; `_build_user_message()` додає секцію документів |
| `orchestrator/pipeline_v2.py` | Передає `state.supporting_docs` до першого виклику `agent1.process()` |
| `main.py` | Нова команда `server` (uvicorn + webbrowser) |
| `requirements.txt` | `fastapi`, `uvicorn[standard]`, `python-multipart`, `pypdf` |

---

*Документ актуальний станом на версію `80e8b32` + Web UI (2026-03-03).*
