...
│   ├── claude_client.py       # ClaudeClient + analyze_cached() + run_agent() + CacheStats
│   ├── legal_texts.py         # Тексти ЦПК/КАС/ГПК/ЗСЗ для Prompt Caching
│   ├── tools.py               # Agent SDK: 6 інструментів + обробники (Phase V2)
...
### Метод та інструменти (Phase V2)

Використовує `run_agent()` з `FEES_TOOLS = [get_fee_rate]`.
Повний текст ЗСЗ (~2 000 токенів) + ключові статті ЦПК + стандарти якості кешуються.
`get_fee_rate` викликається Claude якщо тип вимог неоднозначний.
...
Використовує `run_agent()` з `CRITIC_TOOLS = [search_court_decisions, get_legal_norm, get_procedural_requirements]`.
Claude може перевірити наявність рішень у ChromaDB перед рекомендацією.
...
Використовує `run_agent()` з `GENERATOR_TOOLS = [get_legal_norm, get_procedural_requirements, get_document_template_hints]`.
Claude може запитати точний текст статті перед цитуванням.
...
Використовує `run_agent()` з `EXPERT_TOOLS = [search_court_decisions, get_legal_norm, get_procedural_requirements, validate_document_structure]`.
Claude обов'язково викликає `validate_document_structure` першим, потім перевіряє посилання на практику.

---

## 10. Prompt Caching та Agent SDK

### Два методи ClaudeClient

**`analyze_cached()`** — простий виклик (1 API call):
```python
system = [
...
```
Використовується: Agent 1 (intake_agent.py)

**`run_agent()`** — agentic loop (N API calls, Phase V2):
```python
# Claude може автономно викликати інструменти під час роботи
# Кожен крок циклу використовує той самий cached_system (ephemeral cache)
run_agent(cached_system, dynamic_system, user_message,
          tools=CRITIC_TOOLS, tool_handlers=CRITIC_HANDLERS)
```
Використовується: Agent 2, 3, 4, 5

### Що кешується (однаково в обох методах)

| Агент | Метод | Вміст кешованого блоку | ~Токенів |
|-------|-------|------------------------|----------|
| Agent 1 | `analyze_cached` | Роль + методологія структурування | 2 000 |
| Agent 2 | `run_agent` | Роль + повний текст ЗСЗ + підсудність | 4 000 |
| Agent 3 | `run_agent` | Роль критика + ЦПК + стандарти якості | 6 000 |
| Agent 4 (gen) | `run_agent` | Роль + структура документів + ЦПК/ЗСЗ | 6 500 |
| Agent 4 (compliance) | `analyze_cached` | Роль редактора + ЦПК + КАС + ГПК | 7 000 |
| Agent 5 | `run_agent` | Роль Партнера + стандарти якості | 4 500 |

**Мінімум для кешування:** 1 024 токени. Кеш живе 5 хвилин (ephemeral).
**Економія при 3 ітераціях:** ~80–90% вхідних токенів.
**В agentic loop:** cached_system кешується на кожному кроці → tool_use виклики значно дешевші.
**Тексти кодексів:** [`shared/legal_texts.py`](shared/legal_texts.py)

### Інструменти (Phase V2) — [`shared/tools.py`](shared/tools.py)

| Інструмент | Агенти | Призначення |
|------------|--------|-------------|
| `search_court_decisions` | Agent 3, 5 | Перевірити наявність рішення в ChromaDB |
| `get_legal_norm` | Agent 3, 4, 5 | Отримати точний текст статті закону |
| `get_procedural_requirements` | Agent 3, 4, 5 | Вимоги до типу документа за кодексом |
| `validate_document_structure` | Agent 5 | Regex-перевірка секцій (без API) |
| `get_document_template_hints` | Agent 4 | Підказки структури для doc_type |
| `get_fee_rate` | Agent 2 | Таблиця ставок ЗСЗ |
...
| Agent SDK | Tools викликаються тільки якщо ChromaDB заповнена рішеннями |
...
*Актуально: 2026-03-04 — Phase V2: Claude Agent SDK (tool_use) інтегровано в Agent 2, 3, 4, 5. shared/tools.py (6 інструментів). ClaudeClient.run_agent() — agentic loop з Prompt Caching. anthropic>=0.40.0.*
